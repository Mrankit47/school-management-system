...
const AdminFees = () => {
  ...
  const refreshSelectedWithPayments = async () => {
    ...
    try {
      ...
      setStudentFees((prev) =>
        prev.map((row) =>
          String(row.id) === String(selectedFeeId)
            ? { ...row, ...res.data }
            : row,
        ),
      );
  ...
  const saveStructure = async (e) => {
    ...
    try {
      ...
      if (editingStructureId) {
        await api.patch(
          `fees/admin/structures/${editingStructureId}/`,
          payload,
        );
  ...
  const deleteStructure = async (id) => {
    if (
      !window.confirm(
        "Delete this fee structure? This will also remove linked student fee records for this class.",
      )
    )
      return;
    try {
  ...
  const syncClass = async () => {
    ...
    try {
      const res = await api.post("fees/admin/sync-class/", {
  ...
  const assignStudentFee = async () => {
    ...
    try {
      await api.post("fees/admin/student-fees/create/", {
  ...
  const exportCsv = async () => {
    try {
      ...
      const res = await api.get("fees/admin/export/csv/", {
        params,
        responseType: "blob",
      });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      ...
      >
        <div>
          ...
          <p style={{ margin: "8px 0 0", color: "#6b7280", fontSize: "14px" }}>
            Define fee structure, assign fees, verify student payments, and
            monitor due records.
          </p>
        </div>
        {message ? (
      ...
      >
        <div style={card}>
          ...
          <div style={{ fontSize: "22px", fontWeight: 900 }}>
            ₹{dashboard?.total_fees_scheduled ?? "—"}
          </div>
        </div>
        <div style={card}>
          <div style={labelStyle}>Total Paid</div>
          <div style={{ fontSize: "22px", fontWeight: 900, color: "#166534" }}>
        ...
        <div style={card}>
          ...
          <div style={{ fontSize: "22px", fontWeight: 900, color: "#b91c1c" }}>
            {dashboard?.overdue_records ?? "—"}
          </div>
        </div>
      </div>
          ...
          <div style={card}>
            ...
            {!selectedFeeId ? (
              <p style={{ color: "#6b7280" }}>
                Select a student fee record to view transactions.
              </p>
            ) : (
