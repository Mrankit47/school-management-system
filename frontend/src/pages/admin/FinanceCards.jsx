...
const FinanceCards = () => {
  ...
  const saveCard = async (e) => {
    ...
    try {
      ...
      showMsg(
        firstErr?.class_name?.[0] || firstErr?.error || "Could not create card",
        true,
      );
    }
  };
  ...
  const uploadBulk = async () => {
    ...
    try {
      const res = await api.post("fees/admin/class-fee-cards/upload/", {
        cards: rows,
      });
  ...
  const uploadFromFile = async () => {
    ...
    try {
      const res = await api.post(
        "fees/admin/class-fee-cards/upload-file/",
        fd,
        {
  ...
  const rollbackLastChange = async () => {
    ...
    try {
      ...
      showMsg(
        `${res.data.message}. Restored ${res.data.restored_count} card(s).`,
      );
      await loadCards();
    } catch (e) {
  ...
  const deleteAllCards = async () => {
    if (
      !window.confirm(
        "Delete all fee cards? You can restore using Rollback Last Change.",
      )
    )
      return;
    try {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            ...
            >
              Define and manage class-wise fee structures, bulk upload
              configurations, and financial cards.
            </p>
          </div>
      ...
      {loading && (
        <div style={{ marginBottom: 16, color: "#64748b", fontWeight: 900 }}>
          Processing request...
        </div>
      )}
        ...
        <form onSubmit={saveCard}>
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, registration_fee: e.target.value }))
              }
            />
            <input
              style={fieldStyle}
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, admission_fee: e.target.value }))
              }
            />
            <input
              style={fieldStyle}
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, tuition_fee: e.target.value }))
              }
            />
            <input
              style={fieldStyle}
              ...
              type="number"
              value={form.computer_fee}
              onChange={(e) =>
                setForm((p) => ({ ...p, computer_fee: e.target.value }))
              }
            />
            <input
              style={fieldStyle}
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, annual_charges: e.target.value }))
              }
            />
            <input
              style={fieldStyle}
              placeholder="Science Fee"
              type="number"
              value={form.science_fee}
              onChange={(e) =>
                setForm((p) => ({ ...p, science_fee: e.target.value }))
              }
            />
            <input
              style={fieldStyle}
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, sports_fee: e.target.value }))
              }
            />
          </div>
          <p style={{ marginTop: 10, marginBottom: 8, fontWeight: 700 }}>
            Total Fee: ₹{formTotal.toFixed(2)}
          </p>
          <button
            type="submit"
            className="btn-premium"
      ...
      <div style={{ ...cardStyle, marginTop: 12 }}>
        ...
        <p style={{ color: "#6b7280", fontSize: 13, marginTop: 0 }}>
          Format:
          class,registration,admission,tuition,computer,annual,science,sports
        </p>
        <textarea
          style={{
            width: "100%",
      ...
      <div style={{ ...cardStyle, marginTop: 12 }}>
        ...
        <p style={{ color: "#6b7280", fontSize: 13, marginTop: 0 }}>
          CSV columns: class_name, registration_fee, admission_fee, tuition_fee,
          computer_fee, annual_charges, science_fee, sports_fee
        </p>
        <div
          style={{
            display: "flex",
