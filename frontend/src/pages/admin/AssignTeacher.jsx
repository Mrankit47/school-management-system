...
const AssignTeacher = () => {
  ...
  const fetchSectionsForForm = async (classId) => {
    ...
    try {
      const res = await api.get("classes/admin-sections/", {
  ...
  const fetchSubjectsForForm = async (classId) => {
    ...
    try {
      const res = await api.get("subjects/", {
        params: { class_id: classId, status: "Active" },
      });
  ...
  const filteredTeachers = useMemo(() => {
    ...
    if (formData.subject_id) {
      ...
      if (selectedSubject) {
        ...
        list.sort((a, b) => {
          const aMatch = (a.subject_specialization || "")
            .toLowerCase()
            .includes(subName);
          const bMatch = (b.subject_specialization || "")
            .toLowerCase()
            .includes(subName);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(
        `Error: ${err?.response?.data?.error || "Unable to save assignment."}`,
      );
    } finally {
      setSaving(false);
    }
  ...
  const deleteAssignment = async (id) => {
    ...
    try {
      ...
      setMessage(
        `Error: ${err?.response?.data?.error || "Unable to delete assignment."}`,
      );
    }
  };
    ...
    <div style={{ padding: "20px" }}>
      ...
      >
        <div>
          ...
          >
            Manage which teachers are responsible for which subjects and
            classes.
          </div>
        </div>
        <button
          ...
          >
            ...
            >
              <h2 style={{ margin: 0, fontSize: "20px", fontWeight: 800 }}>
                {editingId ? "Edit Assignment" : "New Teacher Assignment"}
              </h2>
              <button
                onClick={resetForm}
                style={{
                  ...
                  {filteredTeachers.map((t) => {
                    ...
                    return (
                      <option
                        key={t.id}
                        value={t.id}
                        style={{ fontWeight: isSpecialized ? 800 : 400 }}
                      >
