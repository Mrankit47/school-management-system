...
const Exams = () => {
  ...
  useEffect(() => {
    ...
    const sec = sections.find(
      (s) => String(s.id) === String(selectedExam.class_section),
    );
    if (!sec) return;
    ...
    Promise.all([
      ...
      api.get("subjects/", {
        params: { class_id: sec.class_id, status: "Active" },
      }),
    ])
      .then(([schRes, subRes]) => {
  ...
  useEffect(() => {
    ...
    api
      .get(`academics/exams/${selectedExamId}/subject-status/`)
  ...
  const onCreateExam = async (e) => {
    ...
    if (
      !examForm.name ||
      !examForm.class_section ||
      !examForm.start_date ||
      !examForm.end_date ||
      !examForm.total_marks ||
      !examForm.passing_marks
    ) {
  ...
  const addSchedule = async (e) => {
    ...
    const isDuplicate = schedules.some(
      (s) => s.subject === scheduleForm.subject,
    );
    if (isDuplicate) {
  ...
  const saveScheduleEdit = async () => {
    ...
    try {
      await api.patch(
        `academics/schedule/${editingScheduleId}/`,
        editScheduleForm,
      );
      const res = await api.get(`academics/exams/${selectedExamId}/schedule/`);
      setSchedules(res.data || []);
  ...
  const togglePublishResults = async (publish) => {
    ...
    setError("");
    try {
      await api.put(`academics/exams/${selectedExamId}/publish-results/`, {
        publish,
      });
      await refreshExams();
      const statusRes = await api.get(
        `academics/exams/${selectedExamId}/subject-status/`,
      );
  ...
  const saveExamEdit = async () => {
    ...
    if (
      !editExamForm.name ||
      !editExamForm.class_section ||
      !editExamForm.start_date ||
      !editExamForm.end_date
    ) {
  ...
  const deleteExam = async (examId, examName) => {
    const ok = window.confirm(
      `Delete exam "${examName}"? This will also remove schedule and results linked to it.`,
    );
    if (!ok) return;
  ...
  const overviewExams = useMemo(() => {
    return (exams || []).filter((e) => {
      if (
        overviewClassFilter !== "all" &&
        String(e.class_section) !== String(overviewClassFilter)
      )
        return false;
      if (
        overviewStatusFilter !== "all" &&
        String(e.status) !== String(overviewStatusFilter)
      )
        return false;
      if (
        overviewTypeFilter !== "all" &&
        String(e.exam_type) !== String(overviewTypeFilter)
      )
        return false;
      return true;
    });
  ...
  const resultManagementExams = useMemo(() => {
    return (exams || []).filter((e) => {
      if (
        ...
        String(e.class_section) !== String(resultsClassFilter)
      )
        return false;
      return true;
    });
  ...
  useEffect(() => {
    if (!selectedExamId) return;
    const exists = resultManagementExams.some(
      ...
      >
        ...
        <div style={card}>
          ...
          >
            ...
            <div>
              ...
              <select
                value={examForm.class_section}
                onChange={(e) =>
                  setExamForm({ ...examForm, class_section: e.target.value })
                }
                style={input}
                required
              >
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Guidelines for teachers..."
              />
            </div>
            <button
              type="submit"
        ...
        <div style={{ display: "grid", gap: "16px" }}>
          <div style={card}>
            ...
            {!selectedExamId ? (
              <div
                style={{ color: "#6b7280", fontWeight: 800, fontSize: "13px" }}
              >
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        height: "52px",
                        transition: "all 0.3s",
                        boxShadow: `0 8px 20px -6px ${colors.success}66`,
                        minWidth: "130px",
                        whiteSpace: "nowrap",
                        fontSize: "14px",
                      }}
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: "32px" }}>
              ...
              >
                {viewingExam.class_section_display ||
                  `${viewingExam.class_name}-${viewingExam.section_name}`}
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                <div style={{ fontWeight: 800, color: colors.text }}>
                  {viewingExam.start_date} to {viewingExam.end_date}
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div style={{ fontWeight: 800, color: colors.text }}>
                  Passing: {viewingExam.passing_marks} /{" "}
                  {viewingExam.total_marks}
                </div>
              </div>
              <div
                style={{
            ...
            >
              <div className="table-scroll"><table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {(
                    exams.find((ex) => ex.id === viewingExam.id)?.schedules ||
                    []
                  ).length > 0 ? (
                    exams
                      .find((ex) => ex.id === viewingExam.id)
                      .schedules.map((s, idx) => (
                        <tr
                          key={idx}
                        ...
                        >
                          ...
                          <td
                            style={{ padding: "14px 20px", color: colors.text }}
                          >
                            {s.exam_date}
                          </td>
                          <td
                    ...
                    <tr>
                      ...
                      >
                        No schedule defined for this exam yet.
                      </td>
                    </tr>
                  )}
                </tbody>
