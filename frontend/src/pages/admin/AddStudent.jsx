...
const AddStudent = () => {
  ...
  const fatherPhoneDigits = (formData.father_contact || "")
    .replace(/\D/g, "")
    .slice(0, 10);
  const motherPhoneDigits = (formData.mother_contact || "")
    .replace(/\D/g, "")
    .slice(0, 10);
  const selectedSection = mainSections.find(
    (s) => String(s.id) === String(formData.section_id),
  );
  const rollPreview = selectedSection?.name
    ? `101${String(selectedSection.name).trim().charAt(0).toUpperCase()}`
    : "Auto (e.g. 101A)";
  const admissionPreview = (() => {
    const used = new Set(
      (students || [])
        .map((s) => {
          const m = String(s.admission_number || "")
            .toUpperCase()
            .match(/^ADM(\d+)$/);
  ...
  useEffect(() => {
    api
      .get("classes/main-classes/")
      .then((res) => setMainClasses(res.data))
      .catch(() => {});
    api
      .get("classes/main-sections/")
      .then((res) => setMainSections(res.data))
      .catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend requires `username`, but the UI does not have a username input field.
      ...
      payload.father_contact = fatherPhoneDigits
        ? `+91${fatherPhoneDigits}`
        : "";
      payload.mother_contact = motherPhoneDigits
        ? `+91${motherPhoneDigits}`
        : "";
    ...
    <div style={{ padding: "20px" }}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: "18px" }}>
          ...
          <div>
            <div style={labelStyle}>
              Date of Birth (DOB) <span style={{ color: "#dc2626" }}>*</span>
            </div>
            <input
              type="date"
              value={formData.dob}
          ...
          >
            ...
            <div>
              <div style={labelStyle}>
                Mother's Name <span style={{ color: "#dc2626" }}>*</span>
              </div>
              <input
                type="text"
          ...
          >
            <div>
              <div style={labelStyle}>
                Father's Contact <span style={{ color: "#dc2626" }}>*</span>
              </div>
              <div
                style={{
                  display: "grid",
            ...
            <div>
              <div style={labelStyle}>
                Mother's Contact <span style={{ color: "#dc2626" }}>*</span>
              </div>
              <div
                style={{
                  display: "grid",
          ...
          <div>
            <div style={labelStyle}>
              Date of Admission <span style={{ color: "#dc2626" }}>*</span>
            </div>
            <input
              type="date"
              value={formData.date_of_admission}
              onChange={(e) =>
                setFormData({ ...formData, date_of_admission: e.target.value })
              }
              style={inputStyle}
              required
            />
        ...
        {message && (
          <p
            style={{ color: message.startsWith("Error") ? "#dc2626" : "green" }}
          >
            {message}
          </p>
        )}
