...
const AdminHolidays = () => {
  ...
  const classLabel = (hc) =>
    hc?.applicable_classes?.length
      ? hc.applicable_classes.map((c) => c.name).join(", ")
      : "All Classes";
  ...
  const exportCsv = async () => {
    try {
      ...
      const res = await api.get("holidays/export/csv/", {
        params,
        responseType: "blob",
      });
    ...
    <div style={{ padding: "20px" }}>
      ...
      >
        <div>
          ...
          >
            Manage holidays and see them on a calendar.
          </p>
        </div>
                  ...
                  {loading ? (
                    ...
                    sortedHolidays.map((h) => (
                      <tr key={h.id} style={{ borderTop: "1px solid #eef2f7" }}>
                        ...
                        >
                          {h.applicable_classes &&
                          h.applicable_classes.length > 0
                            ? h.applicable_classes.map((c) => c.name).join(", ")
                            : "All Classes"}
                        </td>
                        <td style={{ padding: "12px 10px" }}>
                          <div
            ...
            <div style={{ marginTop: "14px", display: "grid", gap: "12px" }}>
              {(holidayByDay.map.get(selectedDate) || []).map((h) => (
                ...
                >
                  ...
                  >
                    Applies:{" "}
                    {h.applicable_classes && h.applicable_classes.length
                      ? h.applicable_classes.map((c) => c.name).join(", ")
                      : "All Classes"}
                  </div>
                </div>
            ...
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Why is this holiday?"
                />
              </div>

              <div
                style={{ borderTop: "1px solid #eef2f7", paddingTop: "12px" }}
              >
                <div
                  style={{
                    display: "flex",
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setForm({
                          ...form,
                          allClasses: checked,
                          applicable_class_ids: checked
                            ? []
                            : form.applicable_class_ids,
                        });
                      }}
                    />
                    All Classes
