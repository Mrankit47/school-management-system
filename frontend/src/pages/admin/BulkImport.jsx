...
const BulkImport = () => {
  ...
  const handleDeleteHistory = async (logId) => {
    if (
      !window.confirm(
        "Are you sure you want to rollback and DELETE this specific batch of imported users? This cannot be undone.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      alert(
        "Failed to rollback: " + (error.response?.data?.error || error.message),
      );
    }
  };
  ...
  const downloadTemplate = () => {
    ...
    if (importType === "student") {
      csvContent =
        "admission_no,name,username,email,class,section,roll_number,phone\nADM101,John Doe,johndoe,john@example.com,Class 1,A,101,1234567890";
    } else {
      csvContent =
        "employee_id,name,specialization,email,phone,gender,dob,qualification,experience_years,joining_date\nEMP01,Jane Smith,Mathematics,jane@example.com,1234567890,Female,1990-01-01,M.Sc.,5,2023-08-01";
    }
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">
          Bulk Import ({importType === "student" ? "Students" : "Teachers"})
        </h1>
      </div>
                      ...
                      <div className="table-scroll"><table className="min-w-full divide-y divide-green-200">
                        <thead className="bg-green-100">
                          <tr>
                            ...
                            <th className="px-4 py-2 text-right text-xs font-medium text-green-700 uppercase">
                              Action
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-green-200 bg-white">
                          {validRows.slice(0, 50).map((row, idx) => (
                            <tr key={idx}>
                              {importType === "student" && (
                                <td className="px-4 py-2 whitespace-nowrap text-sm text-green-900">
                                  {row.admission_no}
                                </td>
                              )}
                              {importType === "teacher" && (
          ...
          {loadingHistory ? (
            ...
            <div className="overflow-y-auto space-y-4 flex-1 pr-2">
              {history.map((log) => (
                <div
                  key={log.id}
                  className="bg-gray-50 border border-gray-200 rounded p-3 text-sm flex flex-col gap-2"
                >
                  <div className="flex justify-between items-start">
                    <span
                      className="font-semibold text-gray-700 truncate w-3/4"
                      title={log.file_name}
                    >
                      {log.file_name}
                    </span>
