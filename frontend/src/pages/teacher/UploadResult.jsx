...
const UploadResult = () => {
  ...
  const filteredHistory = useMemo(() => {
    return history.filter((item) => {
      const matchSection =
        !listSectionFilter ||
        String(item.class_section) === String(listSectionFilter);
  ...
  const availableExams = useMemo(() => {
    ...
    if (selectedSectionId) {
      filtered = filtered.filter(
        (e) => String(e.class_section) === String(selectedSectionId),
      );
    }
    if (examTypeFilter) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get(
        `academics/class-sections/${selectedSectionId}/teacher-subjects/`,
      ),
    ])
      .then(([studRes, subRes]) => {
  ...
  useEffect(() => {
    ...
    api
      .get(`academics/exams/${examId}/subject-status/`)
  ...
  const handlePublish = async () => {
    ...
    const ok = window.confirm(
      "Are you sure you want to publish these results? This will make them visible to students and lock further edits.",
    );
    if (!ok) return;

    setSubmitting(true);
    try {
      await api.post(`academics/exams/${examId}/publish-results/`, {
        publish: true,
      });
      setIsPublished(true);
          ...
          <div>
            ...
            >
              {view === "form"
                ...
                : "Monitor and track your uploaded records"}
            </p>
          </div>
          ...
          >
            <button
              ...
              style={{
                ...
                boxShadow:
                  view === "form" ? "0 4px 6px -1px rgba(0,0,0,0.1)" : "none",
              }}
            >
              Upload Marks
            </button>
            <button
              ...
              style={{
                ...
                transition: "all 0.2s cubic-bezier(0.4, 0, 0.2, 1)",
                boxShadow:
                  view === "list" ? "0 4px 6px -1px rgba(0,0,0,0.1)" : "none",
              }}
            >
              View History
            </button>
          ...
          <form onSubmit={handleSubmit}>
              ...
              >
                ...
                <select
                  ...
                  style={{
                    ...
                    fontWeight: 600,
                    outline: "none",
                    backgroundColor: !selectedSectionId ? "#f1f5f9" : "#fff",
                  }}
                >
                  <option value="">-- Choose Exam --</option>
                  {availableExams.map((e) => (
                    <option key={e.id} value={String(e.id)}>
                      {e.name} (Max: {e.total_marks})
                    </option>
                  ))}
                </select>
              </div>
              ...
              {selectedStudent ? (
                ...
                >
                  <div>
                    ...
                    >
                      Roll Code: {selectedStudent.roll_number || "N/A"} |
                    ...
                    <div>
                      <input
                        ...
                        style={{
                          ...
                          padding: "16px",
                          borderRadius: "16px",
                          border: `2px solid ${marksErrors[selectedStudentId] ? "#ef4444" : "#e5e7eb"}`,
                          fontSize: "20px",
                          fontWeight: 900,
                          textAlign: "center",
                          outline: "none",
                ...
                >
                  ...
                  <p style={{ marginTop: "8px" }}>
                    Select a student from the menu above to start recording
                    performance
                  </p>
                </div>
              )}
            ...
            >
              {isPublished && (
                <span
                  style={{
                    color: ["unit_test", "class_test"].includes(
                      selectedExam?.exam_type,
                    )
                      ? "#1d4ed8"
                      : "#ef4444",
                    fontWeight: 800,
                    fontSize: "14px",
                    backgroundColor: ["unit_test", "class_test"].includes(
                      selectedExam?.exam_type,
                    )
                      ? "#dbeafe"
                      : "#fef2f2",
