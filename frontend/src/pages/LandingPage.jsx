...
export default function LandingPage() {
  ...
  const { school, loading, error, fetchSchoolInfo, clearSchool } =
    useSchoolStore();
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-inter">
        <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl p-12 text-center animate-in zoom-in-95 duration-500">
          ...
          <p className="text-slate-500 mb-10 leading-relaxed font-medium">
            {error ||
              "We couldn't find the institution you're looking for. Please verify the School ID and try again."}
          </p>
    ...
    <div className="font-inter bg-white text-slate-900 min-h-screen overflow-x-hidden">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between">
          ...
          >
            ...
            <div>
              <p className="text-lg font-black tracking-tight leading-none text-slate-900">
                {school.name}
              </p>
          ...
          <nav className="hidden md:flex items-center gap-10">
            {[
              ...
              .filter((item) => !item.toggle || item.toggle)
              .map((item) => {
                const [label, id] = Array.isArray(item)
                  ? item
                  : [item.label, item.id];
                return (
                  <button
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-2xl border border-slate-100 py-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                {[
                  ...
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        `/school/${school.school_id}/login?role=${role.role}`,
                      )
                    }
      ...
      >
        <div className="absolute inset-0 bg-slate-50">
          <div
            ...
            style={{
              backgroundImage: `url('${school.hero_image || defaultHero}')`,
            }}
          />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 py-24 grid lg:grid-cols-2 gap-16 items-center">
          <div
            ref={heroRef}
            className={`max-w-xl transition-all duration-1000 ${heroInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
          >
            ...
            <h1 className="text-5xl lg:text-7xl font-black text-slate-900 leading-[1.1] mb-6 tracking-tight">
              ...
              <span className="text-blue-600">
                {school.name.split(" ").slice(2).join(" ") || "Institution"}
              </span>
            </h1>
            ...
            <p className="text-base text-slate-500 leading-relaxed mb-10">
              {school.about?.substring(0, 200) ||
                `${school.name} is dedicated to fostering a nurturing environment that empowers students to achieve academic excellence and life-long learning skills.`}
              ...
            </p>
            ...
            <div className="flex flex-wrap gap-4">
              <button
                ...
                className="bg-blue-600 text-white px-10 py-5 rounded-[2rem] text-sm font-bold shadow-2xl shadow-blue-600/30 hover:bg-blue-700 transition-all active:scale-95"
              >
                Explore Programmes
              </button>
              <button
                onClick={() => scrollTo("contact")}
                className="bg-white border border-slate-200 text-slate-900 px-10 py-5 rounded-[2rem] text-sm font-bold hover:bg-slate-50 transition-all active:scale-95"
              >
                Contact Admission
              </button>
            </div>
          ...
          <div className="hidden lg:block relative animate-float">
            <div className="w-[480px] h-[600px] bg-white rounded-[3rem] shadow-2xl p-6 border border-slate-100">
              <img
                src={school.hero_image || defaultHero}
                alt=""
            ...
            <div className="absolute -bottom-10 -left-10 bg-white rounded-3xl shadow-2xl p-8 border border-slate-50 max-w-[240px]">
              ...
              <div className="mt-4 pt-4 border-t border-slate-100">
                <p className="text-xs font-bold text-slate-600">
                  Rated among the top institutions in the region.
                </p>
              </div>
            </div>
          </div>
      ...
      <section ref={statsRef} className="bg-blue-600 py-16">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-12 text-center text-white">
          {[
            {
              val: school.total_students_count || "1.2k+",
              label: "Enrolled Students",
            },
            {
              val: school.total_teachers_count || "85+",
            ...
            <div
              key={i}
              className={`transition-all duration-700 ${statsInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}
      ...
      <section id="about" className="py-24 bg-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 grid lg:grid-cols-2 gap-20 items-center">
          <div
            ref={aboutRef}
            className={`relative transition-all duration-1000 ${aboutInView ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}
          >
            <img
              src={defaultAbout}
              alt=""
              className="w-full h-[540px] object-cover rounded-[3rem] shadow-2xl relative z-10"
            />
          ...
          >
            ...
            <div className="space-y-6 text-slate-600 leading-relaxed text-base font-medium">
              <p>
                {school.about ||
                  `Welcome to ${school.name}, where every student is mentored to reach their full potential. Our comprehensive curriculum is designed to balance academic rigor with creative exploration.`}
              </p>
              <p>
                Founded in {school.established_year || "the late 20th century"},
                our institution has consistently evolved to integrate modern
                technology into traditional teaching methodologies, creating a
            ...
            <div className="mt-12 grid grid-cols-2 gap-8">
              <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                ...
                <p className="text-xs text-slate-500 font-medium">
                  Empowering every learner with future-ready skills.
                </p>
              </div>
              <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                <div className="text-3xl mb-3">🚀</div>
                ...
                <p className="text-xs text-slate-500 font-medium">
                  To lead as a global benchmark for holistic education.
                </p>
              </div>
            </div>
          </div>
      ...
      <section id="academics" className="py-24 bg-slate-50 relative">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div
            ref={acadRef}
            className={`text-center max-w-3xl mx-auto mb-16 transition-all duration-1000 ${acadInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
          >
            ...
            <p className="text-slate-500 font-medium leading-relaxed">
              We follow a globally recognized curriculum structure that ensures
              our students are competitive on both national and international
              platforms.
            </p>
          </div>
            ...
            {[
              {
                ...
                val: school.board || "CBSE / ICSE",
                icon: "🏛️",
              },
              {
                title: "Grade Spectrum",
                val: school.classes_offered || "Nursery - Grade 12",
                icon: "🎓",
              },
              {
                title: "Academic Streams",
                val: school.streams || "Science, Commerce & Arts",
                icon: "🧪",
              },
            ].map((card, i) => (
              <div
                key={i}
                className={`bg-white p-10 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 transition-all duration-700 ${acadInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
                style={{ transitionDelay: `${i * 100}ms` }}
              >
                ...
                <p className="text-slate-500 font-bold tracking-tight text-lg mb-6">
                  {card.val}
                </p>
                <p className="text-sm text-slate-400 leading-relaxed font-medium">
                  Comprehensive educational frameworks designed to prepare
                  students for top-tier universities worldwide.
                </p>
              </div>
            ))}
          </div>
      ...
      <section id="contact" className="py-24 bg-slate-50 relative">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 grid lg:grid-cols-2 gap-16">
          <div
            ref={contRef}
            className={`transition-all duration-1000 ${contInView ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}
          >
            ...
            <div className="space-y-8">
              <div className="flex gap-6 items-start">
                <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-lg text-2xl flex-shrink-0">
                  📍
                </div>
                <div>
                  ...
                  <p className="text-lg font-bold text-slate-700 leading-relaxed">
                    {school.address ||
                      "Main Campus Road, Academic District, India"}
                  </p>
                </div>
              </div>
              <div className="flex gap-6 items-start">
          ...
          <div
            className={`bg-white p-10 lg:p-14 rounded-[3rem] shadow-2xl border border-slate-100 transition-all duration-1000 delay-300 ${contInView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-10"}`}
          >
            {school.google_map_link ? (
              ...
              <form className="space-y-6">
                ...
                <p className="text-slate-500 font-medium text-sm mb-8">
                  Submit your details and our admission counselor will reach out
                  shortly.
                </p>
                <div className="grid grid-cols-2 gap-4">
                  <input
                    ...
                    className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm focus:ring-2 focus:ring-blue-600 transition-all font-medium"
                  />
                  <input
                    type="email"
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 text-sm focus:ring-2 focus:ring-blue-600 transition-all font-medium min-h-[120px] resize-none"
                />
                <button className="w-full bg-blue-600 text-white font-black py-4 rounded-2xl shadow-xl shadow-blue-600/20 hover:bg-blue-700 transition-all">
                  Submit Request
                </button>
              </form>
            )}
      ...
      <footer className="bg-white border-t border-slate-100 py-16">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 grid md:grid-cols-4 gap-12">
          <div className="md:col-span-2">
            ...
            <p className="text-slate-500 font-medium leading-relaxed max-w-sm mb-8">
              A premier educational institute committed to nurturing global
              leaders through academic excellence and holistic development.
            </p>
            <div className="flex gap-4">
              {["TW", "FB", "IG", "LI"].map((soc) => (
                <div
                  key={soc}
                  className="w-10 h-10 bg-slate-50 rounded-full flex items-center justify-center text-[10px] font-black text-slate-400 border border-slate-100 cursor-pointer hover:bg-blue-600 hover:text-white transition-all"
                >
                  {soc}
                </div>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm font-bold text-slate-600">
              <li>
                <button
                  onClick={() =>
                    navigate(`/school/${school.school_id}/login?role=admin`)
                  }
                  className="hover:text-blue-600 transition-colors"
                >
                  Admin Login
              ...
              <li>
                <button
                  onClick={() =>
                    navigate(`/school/${school.school_id}/login?role=teacher`)
                  }
                  className="hover:text-blue-600 transition-colors"
                >
                  Staff Login
              ...
              <li>
                <button
                  onClick={() =>
                    navigate(`/school/${school.school_id}/login?role=student`)
                  }
                  className="hover:text-blue-600 transition-colors"
                >
                  Parent Portal
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 mt-16 pt-8 border-t border-slate-50 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-xs font-bold text-slate-400 italic">
            © {new Date().getFullYear()} {school.name}. Empowering Futures.
          </p>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
            Platform by Atheris Lab
          </p>
        </div>
      </footer>
    </div>
