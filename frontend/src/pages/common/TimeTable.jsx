...
const TimetableGrid = ({
  ...
  const gridData = useMemo(() => {
    const data = {};
    DAYS.forEach((day) => {
      data[day.id] = entries.filter(
        (e) =>
          e.day === day.id &&
          (e.shift_ref === shift?.id || e.shift === shift?.name.toLowerCase()),
      );
    });
    return data;
  }, [entries, shift]);
          ...
          <div className="relative">
            {periods.map((pNum) => {
              return (
                <div
                  key={pNum}
                  className="grid grid-cols-7 border-b border-slate-100 last:border-0"
                >
                            ...
                            <div>
                              <div className="flex items-start justify-between mb-1.5 gap-2">
                                ...
                                <div className="flex flex-col items-end">
                                  ...
                                  <span className="text-[7px] font-black text-slate-300 uppercase mt-0.5">
                                    START
                                  </span>
                                </div>
                              </div>
...
const TimeTable = () => {
  ...
  const selectedShift = useMemo(
    () => shifts.find((sh) => sh.id === parseInt(selectedShiftId)),
    ...
    const existing = entries.find(
      (e) =>
        ...
        (e.shift_ref === parseInt(selectedShiftId) ||
          e.shift === selectedShift?.name?.toLowerCase()) &&
            ...
            {shifts.map((opt) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                  selectedShiftId === opt.id.toString()
      ...
      {isAdmin && !filters.class_name && !selectedTeacherId ? (
        <div className="bg-white rounded-3xl border border-dashed border-slate-200 p-12 text-center">
          ...
          <p className="text-slate-500 text-sm mt-1 max-w-xs mx-auto">
            Please use the filters above to view and manage class schedules.
          </p>
        </div>
      ) : loading ? (
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in zoom-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden relative border border-slate-100">
            <button
              ...
              className="absolute top-6 right-6 p-2.5 bg-slate-50 hover:bg-slate-100 rounded-2xl text-slate-400 transition-all z-10"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="view-modal-content p-6 pt-2">
              <div className="text-center mb-5">
                <h3 className="text-xl font-black text-slate-800 tracking-tight">
                  {viewingEntry.subject}
                </h3>
                <p className="text-slate-500 font-bold mt-0.5 uppercase tracking-widest text-[9px]">
                  Period {viewingEntry.period_number || viewingEntry.period}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50/50 border border-slate-100/50">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-slate-700 text-sm">
                      {viewingEntry.class_name} - {viewingEntry.section}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50/50 border border-slate-100/50">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-slate-700 text-sm">
                        {DAYS.find((d) => d.id === viewingEntry.day)?.name}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 rounded-3xl bg-school-blue/5 border border-school-blue/10 flex items-center justify-between">
                  <div className="text-center flex-1">
                    <p className="text-[8px] font-black text-school-blue uppercase tracking-widest mb-0.5 opacity-60">
                      Starts at
                    </p>
                    <p className="text-lg font-black text-school-blue">
                      {viewingEntry.start_time_display}
                    </p>
                  </div>
              ...
              <button
                onClick={() => setViewingEntry(null)}
                className="w-full mt-6 py-3 bg-slate-800 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl shadow-slate-200"
              >
                Close Details
              </button>
            </div>
...
const AdminModal = ({
  ...
  useEffect(() => {
    if (
      ...
      formData.start_time === "00:00:00"
    ) {
      const pNum = formData.period_number;
      const isMorning =
        selectedShift?.name.toLowerCase().includes("morning") ||
        formData.shift_ref === 1;
  ...
  const filteredSubjects = useMemo(() => {
    ...
    return (meta.subjects || []).filter(
      (s) => s.class_name === formData.class_name,
    );
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
        <div className="p-4 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-white rounded-xl text-slate-400 transition-all"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    shift_ref: parseInt(e.target.value),
                  })
                }
                className="w-full px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold text-slate-700 outline-none focus:border-school-blue focus:ring-2 focus:ring-school-blue/10 transition-all"
                required
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    period_number: parseInt(e.target.value, 10),
                  })
                }
                className="w-full px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold text-slate-700 outline-none focus:border-school-blue focus:ring-2 focus:ring-school-blue/10 transition-all"
              >
              ...
              value={
                meta.sections.find(
                  (s) =>
                    ...
                    s.section_name === formData.section,
                )?.id || ""
              }
              className="w-full px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold text-slate-700 outline-none focus:border-school-blue focus:ring-2 focus:ring-school-blue/10 transition-all"
              required
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl p-6">
        ...
        <form
          ...
          className="space-y-4 mb-8 p-4 bg-slate-50 rounded-2xl border border-slate-100"
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-school-blue text-white rounded-xl font-bold shadow-lg shadow-school-blue/20"
          >
            {loading
              ? "Saving..."
              : editingShift
        ...
        <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
          {shifts.map((s) => (
            <div
              key={s.id}
              className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl group hover:border-school-blue/30 transition-all"
            >
              <div>
              ...
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                <button
                  ...
                  className="p-2 text-slate-400 hover:text-school-blue hover:bg-school-blue/10 rounded-xl transition-all"
                >
                  <Edit2 className="w-4 h-4" />
                </button>
              </div>
