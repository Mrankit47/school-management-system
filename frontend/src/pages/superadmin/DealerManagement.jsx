  ...
  const handleViewDealer = async (dealer) => {
    ...
    try {
      const response = await api.get(
        `/dealers/management/${dealer.id}/schools/`,
      );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            <button
              ...
              className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-2 flex items-center gap-1 hover:underline"
            >
              ← Back to Overview
            </button>
            ...
            <p className="text-slate-500 mt-1 text-sm font-medium">
              Manage independent platform partners and their territories
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {dealers.map((dealer) => (
            <div
              key={dealer.id}
              className="bg-white border border-slate-200 p-8 rounded-[2rem] shadow-sm hover:shadow-xl transition-all duration-500 group"
            >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl p-12 relative max-h-[90vh] overflow-y-auto">
              ...
              <p className="text-slate-500 mb-10 text-sm font-medium">
                Create a new independent dealer account and profile.
              </p>
              ...
              <form onSubmit={handleCreateDealer} className="space-y-8">
                ...
                <div className="pt-4">
                  <button
                    type="submit"
                    className="w-full bg-slate-900 text-white font-bold py-5 rounded-2xl shadow-2xl transition-all active:scale-[0.98] text-sm tracking-wide"
                  >
              ...
              <div className="flex items-center gap-6 mb-10">
                ...
                <div>
                  ...
                  <p className="text-indigo-600 font-bold text-[10px] uppercase tracking-widest mt-1">
                    Territory: {viewingDealer.location}
                  </p>
                </div>
              </div>
                  ...
                  {dealerSchools.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {dealerSchools.map((school, idx) => (
                        <div
                          key={idx}
                          className="bg-slate-50 rounded-2xl p-6 border border-slate-100 flex items-center justify-between"
                        >
                          <div>
                    ...
                    <div className="p-12 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                        No schools registered by this dealer.
                      </p>
                    </div>
                  )}
                </div>
